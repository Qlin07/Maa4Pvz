    ...
    for (const std::string& task_name : m_tasks_name) {
        ...
        case AlgorithmType::OcrDetect:
            if (auto ocr_opt = ocr(task_ptr)) {
                Log.trace(__FUNCTION__, "| OcrDetect", task_ptr->name);
...
FeatureMatcher::ResultsVecOpt PipelineAnalyzer::feature_match(const std::shared_ptr<TaskInfo>& task_ptr) const
{
